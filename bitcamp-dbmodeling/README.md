...
- 후보키(candidate key) 선정
  - key 들 중에서 중복되는 컬럼을 제거한 후 최소키를 선정한다.
...
- 테이블 간의 관계를 맺는다.
- 다른 테이블의 데이터를 참조하는 경우 그 테이블과 관계를 연결한다.
- 참조하는 테이블이 자식 테이블이고, 참조 당하는 테이블이 부모 테이블이다.
  즉 해당 컬럼을 관계를 표현하는 FK 컬럼으로 전환한다.
- 자식 테이블 쪽에 참조하는 부모 테이블의 PK 컬럼을 놓는다.
- 자식 테이블에 놓은 부모 테이블의 PK 값을 '외부키(foreign key)'라 부른다.
...
### 10. 포함 관계 및 배타적 관계 추가
- 여러 테이블에 공통으로 포함되는 컬럼이 있는 경우, 별도의 테이블로 분리하여 정의한다.
  다음 중 하나의 관계로 설정한다.
