...
pool.getConnection(function(err, con) {
    ...
    con.query('select * from pms2_member', function(err, results) {
        ...
        // 프로그램을 종료하고 싶다면 작업이 끝난 후 connectionPool의 모든 connection을 닫아야 한다.
        // 보통 서버로서 실행하다가 종료할 때 connectionPool을 닫는다.
        // 커넥션 풀을 닫지 않으면 Node.js의 메인 스레드가 종료되지 않는다.
        // 물론 서버로 실행할 때는 당연히 그래야 하지만,
        // 이 프로그램 예제 처럼 그냥 간단히 사용하고 종료하고 싶다면,
        // 다음과 같이 질의를 완료한 후에 connectionPool을 닫아라.
        // => 이런 단일 프로그램은 connectionPool을 사용할 필요가 없다.
