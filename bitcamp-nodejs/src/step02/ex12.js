// 주제: 데이터베이스 프로그래밍 - connection pool 사용 I
...
// => pool.connection() -> con.query() -> con.release()의 단축 문법
pool.query('select * from pms2_member', function(err, results) {
        ...
        // 이 예제에서는 sql 수행한 후에 프로그램을 종료해야 학 ㅣ때문에 
        // connectionPool을 닫는다.
        pool.end();        
});
...
// [ex12.js]
// => ex12.js 는 커넥션 풀에 대해 바로 query()를 호출할 수 있어
//    코드가 간결해져서 좋다.
...
// => 그러나 한 번 connection을 얻으면 여러 번 사용할 수 있기 때문에
//    여러개의 sql을 실행할 때 ex12.js보다는 유리하다.
// => 예)
//     pool.getConnection(function(err, con){
//          con.query(...); // 커넥션1
//          con.query(...); // 커넥션1 같은 커넥션
//     })
// 여러개의 작업 동시에 할 시
